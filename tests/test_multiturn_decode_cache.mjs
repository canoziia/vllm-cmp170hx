...
for(const max of [64,256,1024,1500]){
 ...
 console.log(JSON.stringify({test:'responses',max,first:first.usage,second:second.usage,shared_recompute:oldTotal-hit}));assert(oldTotal-hit<=160,'shared recomputation exceeds 128-token snapshot interval + MTP32 margin');
}
const batch=await Promise.all(Array.from({length:32},async(_,i)=>{
 ...
 assert(rem<=160,`concurrent cache miss ${rem}`);return {i,rem,usage:b.usage};}));console.log(JSON.stringify({test:'concurrent',batch}));
