...
## Hot-OFF telemetry A/B and controlled historical ShadowSource A/B

Five consecutive hot-OFF c1 runs on the same live model (one warmup plus four measured, 53 prompt/1000 completion tokens, debug off) produced Full-batch 31.62 warmup then **21.31, 31.57, 24.12, 36.81 tok/s**. Runs 1 and 3 had 273 and 175 gaps >60ms; runs 2 and 4 had none. One-Hz GPU telemetry showed stable SM clocks (GPU0/1/4 about 1485MHz, GPU2/3 about 1470MHz, GPU5 1140MHz), rejecting a clock-collapse explanation. In slow segments GPU0/1 stayed ~96% utilized while downstream utilization fell (representative rank2 73% and rank5 15%, versus fast rank2 97% and rank5 26%); the symptom is upstream/downstream pipeline starvation, not a uniformly slow GPU. Raw data: `off-telemetry-ab/` and `gpu.csv`.

The older controlled Zero-Engram matrix varied ShadowSource count and PP partition while holding source revision and plain/DSpark settings fixed. Plain long c1 was 40.94–41.31 (2 shadows), 38.97–39.98 (3), and 40.67–41.63 tok/s (4); DSpark step rates were 22.73–22.94, 22.78–23.03, and 22.48–22.70. Thus shadow count alone does not explain either current K0 variability or the ~24-step/s DSpark ceiling; stage partition matters but no monotonic L14/L20 source-count penalty is present. This is an A/B on Zero Engram and cannot quantify current CPU Engram UVA cost.

## Hot-K0 feedback optimization candidate (not deployed)

Source now skips the draft-token feedback collective symmetrically on all PP ranks when the scheduler supplies `next_draft_k == 0`; sampled target tokens and sampled/rejected counts still use the two required feedback broadcasts. ON K5 remains three broadcasts. The last rank also skips `broadcast_drafts` at K0. A mocked protocol test inside the pinned runtime image verifies K0=2 collectives, K5=3, and the deferred queue contains no draft tensor only at K0; fresh patch apply, syntax checks and diff checks pass. This is shape-safe because K0 is identical on all ranks and no target model padding/Graph dispatch changes. It is **not deployed into the healthy full model and has no throughput A/B yet**; existing trace puts the raw draft broadcast Event near 0.03–0.8ms, so it cannot honestly explain the entire hot-OFF gap.
