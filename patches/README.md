...
| `0009-mtp-safe-partial-tail-checkpoint.patch` | Save recurrent partial-tail state at the boundary restored after MTP rollback, register the final Full Attention boundary as a drop-only marker when necessary, and let EAGLE/MTP lookup inspect that one marker unit beyond the public hit cap. |
